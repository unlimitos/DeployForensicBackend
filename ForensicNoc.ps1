#############################################################################################
#region   Defend global variable   ##########################################################
#############################################################################################
$DateFormat = "yyyy'_'MM'_'dd"
$SaveTime = Get-Date -Format $DateFormat
$FtpServerAddress = "10.24.5.111"

# folder on FTP server
$FtpUploadFolder = "/01_NOC_MT/ANM/"

$FtpUser = "anonymous"
$FtpPass = "anonymous"
$FtpPath = "ftp://" + $FtpServerAddress + $FtpUploadFolder + $SaveTime

# $IsUploadFtp = true will upload result to FTP server, false bypass upload
$IsUploadFtp = $false
# IsRemoveFile = true will remove all files from client after upload to FTP server, false don't remove file
$IsRemoveFile = $false

# CheckingPeriods = x (weeks) : Periods for checking time. It used for config task scheduled.
# RunTime  : time run task
$CheckingPeriods = 4
$RunTime = "4am"


#Post data through Rest API
$IsPostData = $true
$PostURI = "http://10.27.0.2:9191/v1/forensicatorlog"
#endregion
#############################################################################################################
#region   Function COMPRESS FILES & UPLOAD FILE    ##########################################################
#############################################################################################################


function ValidateFtpFolder {
    Param 
    (
        [Parameter(Mandatory=$true, Position=0)]
        [string] $FtpUserName,
        [Parameter(Mandatory=$true, Position=1)]
        [string] $FtpPass,
        [Parameter(Mandatory=$true, Position=2)]
        [string] $Path

    )
    try {
        $MakeDirectory = [System.Net.WebRequest]::Create($Path)
        $MakeDirectory.Credentials = New-Object System.Net.NetworkCredential($FtpUserName,$FtpPass)
        $MakeDirectory.Method = [System.Net.WebRequestMethods+FTP]::MakeDirectory
        $MakeDirectory.GetResponse()
    }
    catch [Net.WebException]
    {
        Write-Host -Fore Cyan "Ftp folder ready exits"
    }
}

function UploadFtpServer {
    Param 
    (
        [Parameter(Mandatory=$true, Position=0)]
        [string] $FtpUserName,
        [Parameter(Mandatory=$true, Position=1)]
        [string] $FtpPass,
        [Parameter(Mandatory=$true, Position=2)]
        [string] $Path

    )
    Write-Host -Fore Cyan "[!] All Done... you can find the results in the script execution folder"

    cd $PSScriptRoot
    $SourceZip = Get-ChildItem -Directory | Select -ExpandProperty Name
    $DestZip = './' + $Pc_Name + '.zip'
    Compress-Archive -Path $SourceZip  -DestinationPath $DestZip -CompressionLevel Optimal

    $SourceDir = Get-Location |  Select -ExpandProperty Path

    $ftp = $Path + "/" + $Pc_Name + '.zip' 
    
    $FileZip = $SourceDir + "\" + $Pc_Name + '.zip'
    $webClient = New-Object -TypeName System.Net.WebClient
    $webClient.Credentials = New-Object System.Net.NetworkCredential($FtpUserName,$FtpPass)
    $uri = New-Object -TypeName System.Uri -ArgumentList $ftp
    $webClient.UploadFile($uri,$FileZip)
    $webClient.Dispose()
    if ( $IsRemoveFile -eq $true) {
        Remove-Item -Path $FileZip
        $Del_result = $SourceDir + "\" + $Pc_Name
        Get-ChildItem -Path $Del_result -File -Recurse | Remove-Item
    }
    
}


function CreateTask {
    Param 
    (
        [Parameter(Mandatory=$true, Position=0)]
        [int] $priods,
        [Parameter(Mandatory=$true, Position=1)]
        [string] $runtime
    )
    Write-Host -Fore DarkCyan   '[!] Create task'
    $Current_Path = Get-Location | Select -ExpandProperty Path
    $Triger = New-ScheduledTaskTrigger -Weekly -WeeksInterval $priods -DaysOfWeek Sunday -At $runtime
    $dfSettings = New-ScheduledTaskSettingsSet
    $Action = New-ScheduledTaskAction -Execute 'powershell.exe' -WorkingDirectory $Current_Path -Argument '-NonInteractive -NoLogo -NoProfile -ExecutionPolicy Bypass -File ".\Forensicator.ps1"'
    $hcTask = New-ScheduledTask -Action $Action -Trigger $Triger -Settings $dfSettings
}

function PostData {
    Param 
    (
        [Parameter(Mandatory=$true, Position=0)]
        [string] $uri,
        [Parameter(Mandatory=$true, Position=1)]
        [Object] $Data,
        [Parameter(Mandatory=$true, Position=2)]
        [boolean] $IsPostData,
        [Parameter(Mandatory=$true, Position=3)]
        [string] $IndexName,
        [Parameter(Mandatory=$true, Position=4)]
        [string] $GroupName,
        [Parameter(Mandatory=$true, Position=5)]
        [string] $ComputerName
    )
    if ( $IsPostData -eq $true) {
        #$postDataObject = [PSCustomObject]@{
        #    "DataType"=$DataType
        #    "Values"= @($Data)
        #}
        $bodyData = ConvertTo-Json  -InputObject @($Data)
        $headers = @{}
        $headers.Add("IndexName",$IndexName)
        $headers.Add("GroupName",$GroupName)
        $headers.Add("ComputerName",$ComputerName)
        Invoke-RestMethod -uri $uri -Method Post -Body $bodyData -ContentType "application/json" -Headers $headers
        }
}  

#endregion

##################################################
#region             LOGO                         #
##################################################
$MyVersion = '1.0.0'
$t = @"
       _____          ___.    .__   _____                       
      /     \    ____ \_ |__  |__|_/ ____\____    ____    ____  
     /  \ /  \  /  _ \ | __ \ |  |\   __\/  _ \  /    \ _/ __ \ 
    /    Y    \(  <_> )| \_\ \|  | |  | (  <_> )|   |  \\  ___/ 
    \____|__  / \____/ |___  /|__| |__|  \____/ |___|  / \___  >
            \/             \/                        \/      \/ 
                                                               
___________                                .__               __                
\_   _____/__________   ____   ____   _____|__| ____ _____ _/  |_  ___________ 
 |    __)/  _ \_  __ \_/ __ \ /    \ /  ___/  |/ ___\\__  \\   __\/  _ \_  __ \
 |     \(  <_> )  | \/\  ___/|   |  \\___ \|  \  \___ / __ \|  | (  <_> )  | \/
 \___  / \____/|__|    \___  >___|  /____  >__|\___  >____  /__|  \____/|__|   
     \/                    \/     \/     \/        \/     \/                    

                                                                          $MyVersion

"@

for ($i=0;$i -lt $t.length;$i++) {
if ($i%2) {
 $c = "red"
}
elseif ($i%5) {
 $c = "yellow"
}
elseif ($i%7) {
 $c = "green"
}
else {
   $c = "white"
}
write-host $t[$i] -NoNewline -ForegroundColor $c
}

#######################################################################
#region PARAMETER SETTINGS  ###########################################
#######################################################################

$InternalIp = Get-NetIPaddress | select InterfaceAlias, IPaddress |  ? InterfaceAlias -eq "Internal" | select -exp IPaddress

$Pc_Name = $env:COMPUTERNAME + "_" + $InternalIp
$OPERATOR = $env:COMPUTERNAME
$CASE = $env:COMPUTERNAME
$TITLE = 'HealthCheck ' + $env:COMPUTERNAME
$LOCATION = $env:COMPUTERNAME
$DEVICE = $env:COMPUTERNAME


#CreateTask $CheckingPeriods $RunTime

if ($OPERATOR) {
   
   $Handler = $OPERATOR
   
} 
else {
	
   $Handler = Read-Host -Prompt 'Enter Investigator Name'	

}

#FOR CASE REFERENCE
if ($CASE) {
   
   $CASENO = $CASE
   
} 
else {
	
   $CASENO = Read-Host -Prompt 'Enter Case Reference'

}

#EXHIBIT REFERENCE
if ($TITLE) {
   
   $Ref = $TITLE
   
} 
else {
	
   $Ref = Read-Host -Prompt 'Enter Investigation Title'

}

#LOCATION
if ($LOCATION) {
   
   $Loc = $LOCATION
   
} 
else {
	
   $Loc = Read-Host -Prompt 'Enter examination location'

}

#DESCRIPTION
if ($DEVICE) {
   
   $Des = $DEVICE
   
} 
else {
	
   $Des = Read-Host -Prompt 'Enter description of device e.g. "Asus Laptop"'

}



$ErrorActionPreference= 'silentlycontinue'

#######################################################################
#endregion END PARAMETER SETTINGS #####################################
#######################################################################

Write-Host ''
Write-Host ''
Write-Host ''

$DateFormat = "yyyy'-'MM'-'dd HH':'mm':'ss"
$StartTime = Get-Date -Format $DateFormat

# creating a directory to store the artifacts of 	this host
mkdir $env:computername -Force | Out-Null

# Moving to the new folder
Set-Location $env:computername

# Setting index output file
$FinalDes = 'index.html'

# Setting Processes Output
$ProcDes = 'processes.html'

# Setting Evtx Checks Output
$EVTXDes = 'evtx.html'

# Setting Network Information Output
$NetDes = 'network.html'

# Setting Users Information Output
$UserDes = 'users.html'

# Setting Users Information Output
$InstallProDes = 'installed.html'

# Setting Other Checks Output
$OtherDes = 'others.html'

##################################################
#region     EVENT LOG ANALYSIS		     	     #
##################################################

Write-Host -Fore DarkCyan "[*] Entering Event Log Analysis Mode"

##### USER RELATED CHECKS ##########
##### Get Date for check Log #######
$EndTime = Get-Date
$StartTime = $EndTime.AddDays(-35)


###############################################################################
###  Check Log Unkown service event 01 ########################################
###############################################################################
$ServiceLogs = @(
 '7045'
)
$ServiceEventFilter = @{LogName='System';ProviderName='Service Control Manager';ID=$ServiceLogs; StartTime = $StartTime; EndTime=$EndTime  } 
$ServiceLogsEvent = Get-WinEvent -FilterHashtable $ServiceEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $ServiceLogsEvent $IsPostData "ServiceLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Delete Log  event 02 ###############################################
###############################################################################
$DeleteLogs = @(
 '104', '1102'
)
$DeleteLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$DeleteLogs; StartTime = $StartTime; EndTime=$EndTime  } 
$DeleteLogsEvent = Get-WinEvent -FilterHashtable $DeleteLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     
PostData $PostURI $DeleteLogsEvent $IsPostData "DeleteLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Stop Service Event 03 ##############################################
###############################################################################
$StopServiceLogs = @(
 '6006'
)
$StopServiceLogsEventFilter = @{LogName='System';ID=$StopServiceLogs; StartTime = $StartTime; EndTime=$EndTime  } 
$StopServiceLogsEvent = Get-WinEvent -FilterHashtable $StopServiceLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $StopServiceLogsEvent $IsPostData "StopServiceLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check UnkownCommand Event 04 #############################################
###############################################################################
$UnkownCommandParameterLogs = @(
 '4688'
)
$UnkownCommandParameterLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$UnkownCommandParameterLogs; StartTime = $StartTime; EndTime=$EndTime  } 
$UnkownCommandParameterLogsEvent = Get-WinEvent -FilterHashtable $UnkownCommandParameterLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $UnkownCommandParameterLogsEvent $IsPostData "UnkownCommandParameterLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check PowershellCommand Event 05 #########################################
###############################################################################
$PowershellCommandLogs = @(
 '800','600','4100','4101','4102','4103','4104','4105','4106','4107','4108','4109'
)
$PowershellCommandLogsEventFilter = @{LogName='Windows PowerShell';ProviderName='PowerShell';ID=$PowershellCommandLogs; StartTime = $StartTime; EndTime=$EndTime } 
$PowershellCommandLogsEvent = Get-WinEvent -FilterHashtable $PowershellCommandLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $PowershellCommandLogsEvent $IsPostData "PowershellEventCommandLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Register Event 06 #########################################
###############################################################################
$RegistryLogs = @(
 '4657'
)
$RegistryLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$RegistryLogs; StartTime = $StartTime; EndTime=$EndTime } 
$RegistryLogsEvent = Get-WinEvent -FilterHashtable $RegistryLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $RegistryLogsEvent $IsPostData "RegistryLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check File log Event 07 ##################################################
###############################################################################
$FileLogs = @(
 '4663','4656','4658'
)
$FileLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$FileLogs; StartTime = $StartTime; EndTime=$EndTime } 
$FileLogsEvent = Get-WinEvent -FilterHashtable $FileLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $FileLogsEvent $IsPostData "FileLogs" "EvenLogs" $env:COMPUTERNAME


###############################################################################
###  Check Share log resource Event 08 ########################################
###############################################################################
$ShareResourceLogs = @(
 '5140','5145'
)
$ShareResourceLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$ShareResourceLogs; StartTime = $StartTime; EndTime=$EndTime } 
$ShareResourceLogsEvent = Get-WinEvent -FilterHashtable $ShareResourceLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $ShareResourceLogsEvent $IsPostData "ShareResourceLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Connect from PC to other PC Event 09 ###############################
###############################################################################
$WMIConnectLogs = @(
 '5861'
)
$WMIConnectLogsEventFilter = @{LogName='Security';ID=$WMIConnectLogs; StartTime = $StartTime; EndTime=$EndTime } 
$WMIConnectLogsEvent = Get-WinEvent -FilterHashtable $WMIConnectLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $WMIConnectLogsEvent $IsPostData "WMIConnectLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Task schedule Event 10 ###############################
###############################################################################
$TaskcheduleLogs = @(
 '106'
)
$TaskcheduleLogsEventFilter = @{LogName='System';ID=$TaskcheduleLogs; StartTime = $StartTime; EndTime=$EndTime } 
$TaskcheduleLogsEvent = Get-WinEvent -FilterHashtable $TaskcheduleLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $TaskcheduleLogsEvent $IsPostData "TaskcheduleLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check action ScTcbPrivilege Event 11 ###############################
###############################################################################
$ScTcbPrivilege = @(
 '4703'
)
$ScTcbPrivilegeEventFilter = @{LogName='Security';ID=$ScTcbPrivilege; StartTime = $StartTime; EndTime=$EndTime  } 
$ScTcbPrivilegeEvent = Get-WinEvent -FilterHashtable $ScTcbPrivilegeEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $ScTcbPrivilegeEvent $IsPostData "ScTcbPrivilege" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Task schedule Event 12 ###############################
###############################################################################
$LogonLogs = @(
 '4624'
)
$LogonLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$LogonLogs; StartTime = $StartTime; EndTime=$EndTime } 
$LogonLogsEvent = Get-WinEvent -FilterHashtable $LogonLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $LogonLogsEvent $IsPostData "LogonLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Task schedule Event 13 ###############################
###############################################################################
$LogonFailLogs = @(
 '4625'
)
$LogonFailLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$LogonFailLogs; StartTime = $StartTime; EndTime=$EndTime } 
$LogonFailLogsEvent = Get-WinEvent -FilterHashtable $LogonFailLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $LogonFailLogsEvent $IsPostData "LogonFailLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Task schedule Event 14 #############################################
###############################################################################
$ChangeUserLogs = @(
 '4738','4762'
)
$ChangeUserLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$ChangeUserLogs; StartTime = $StartTime; EndTime=$EndTime } 
$ChangeUserLogsEvent = Get-WinEvent -FilterHashtable $ChangeUserLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $ChangeUserLogsEvent $IsPostData "ChangeUserLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Task schedule Event 15 #############################################
###############################################################################
$ChangeGroupLogs = @(
 '4738','4762'
)
$ChangeGroupLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$ChangeGroupLogs; StartTime = $StartTime; EndTime=$EndTime } 
$ChangeGroupLogsEvent = Get-WinEvent -FilterHashtable $ChangeGroupLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $ChangeGroupLogsEvent $IsPostData "ChangeGroupLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Task schedule Event 16 #############################################
###############################################################################
$ActiveDirectoryLogs = @(
 '4661'
)
$ActiveDirectoryLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$ActiveDirectoryLogs; StartTime = $StartTime; EndTime=$EndTime } 
$ActiveDirectoryLogsEvent = Get-WinEvent -FilterHashtable $ActiveDirectoryLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $ActiveDirectoryLogsEvent $IsPostData "ActiveDirectoryLogs" "EvenLogs" $env:COMPUTERNAME

###############################################################################
###  Check Task schedule Event 17 #############################################
###############################################################################
$ChangeTimeLogs = @(
 '4616'
)
$ChangeTimeLogsEventFilter = @{LogName='Security';ProviderName='Microsoft-Windows-Security-Auditing';ID=$ChangeTimeLogs; StartTime = $StartTime; EndTime=$EndTime } 
$ChangeTimeLogsEvent = Get-WinEvent -FilterHashtable $ChangeTimeLogsEventFilter -ErrorAction $ErrorActionPreference | Select Id,Message,ProviderName,LogName,ProcessId,MachineName, @{Name='UserId';Expression={$_.UserId.Value}},@{Name='TimeCreated';Expression={$_.TimeCreated.ToString("yyyy/MM/dd HH:mm:ss")}}     

PostData $PostURI $ChangeTimeLogsEvent $IsPostData "ChangeTimeLogs" "EvenLogs" $env:COMPUTERNAME


##################################################
#End Region     EVENT LOG ANALYSIS		     	 #
##################################################

Write-Host -Fore Cyan "[!] Done"

###############################################################
#region Live Running Processes & Scheduled Tasks  19,20,21,22 #
###############################################################

Write-Host -Fore DarkCyan "[*] Gathering Processes and Tasks"
#Services 19
$Services = Get-Service | Select-Object Name, DisplayName, Status, StartType 
PostData $PostURI $Services $IsPostData "SystemServices" "Processes_Schedule" $env:COMPUTERNAME

#Process 20
$Processes = Get-Process | Select Handles,@{Name='StartTime';Expression={$_.StartTime.ToString("yyyy/MM/dd HH:mm:ss")}}, id, ProcessName, Path, Product, FileVersion
PostData $PostURI $Processes $IsPostData "SystemProcess" "Processes_Schedule" $env:COMPUTERNAME
#Items set to run on startup 21

$StartupProgs = Get-WmiObject Win32_StartupCommand | select Command, User, Caption 
PostData $PostURI $StartupProgs $IsPostData "StartupProgs" "Processes_Schedule" $env:COMPUTERNAME

# Scheduled Tasks 22
$ScheduledTask = Get-ScheduledTask | ? State -eq running | select TaskPath,TaskName,State
PostData $PostURI $ScheduledTask $IsPostData "ScheduledTask" "Processes_Schedule" $env:COMPUTERNAME

# Get Running Tasks and Their state
$ScheduledTask2 = Get-ScheduledTask | ? State -eq running | Get-ScheduledTaskInfo | select @{Name='LastRunTime';Expression={$_.LastRunTime.ToString("yyyy/MM/dd HH:mm:ss")}},LastTaskResult,@{Name='NextRunTime';Expression={$_.NextRunTime.ToString("yyyy/MM/dd HH:mm:ss")}},NumberOfMissedRuns,TaskName,TaskPath,PSComputerName
PostData $PostURI $ScheduledTask2 $IsPostData "ScheduledTask2" "Processes_Schedule" $env:COMPUTERNAME


Write-Host -Fore Cyan "[!] Done"

#endregion


##################################################
#region Network Information and Settings         #
##################################################
Write-Host -Fore DarkCyan "[*] Gathering Network & Network Settings"

#Gets DNS cache. Replaces ipconfig /dislaydns

$DNSCache = Get-DnsClientCache | select Entry,Name, Status, TimeToLive, Data
PostData $PostURI $DNSCache $IsPostData "DNSCache" "Networks" $env:COMPUTERNAME

$NetworkAdapter = Get-WmiObject -class Win32_NetworkAdapter  | Select-Object -Property AdapterType,ProductName,Description,MACAddress,Availability,NetconnectionStatus,NetEnabled,PhysicalAdapter
PostData $PostURI $NetworkAdapter $IsPostData "NetworkAdapter" "Networks" $env:COMPUTERNAME

#Replaces ipconfig:

$IPConfiguration = Get-WmiObject Win32_NetworkAdapterConfiguration |  select Description, @{Name='IpAddress';Expression={$_.IpAddress -join '; '}}, @{Name='IpSubnet';Expression={$_.IpSubnet -join '; '}}, MACAddress, @{Name='DefaultIPGateway';Expression={$_.DefaultIPGateway -join '; '}}, DNSDomain, DNSHostName, DHCPEnabled, ServiceName
PostData $PostURI $IPConfiguration $IsPostData "IPConfiguration" "Networks" $env:COMPUTERNAME

$NetIPAddress = Get-NetIPaddress | select InterfaceAlias, IPaddress, EnabledState, OperatingStatus 
PostData $PostURI $NetIPAddress $IsPostData "NetIPAddress" "Networks" $env:COMPUTERNAME

$NetConnectProfile = Get-NetConnectionProfile | select Name, InterfaceAlias, NetworkCategory, IPV4Connectivity, IPv6Connectivity
PostData $PostURI $NetConnectProfile $IsPostData "NetConnectProfile" "Networks" $env:COMPUTERNAME

$NetAdapter = Get-NetAdapter | select Name, InterfaceDescription, Status, MacAddress, LinkSpeed 
PostData $PostURI $NetAdapter $IsPostData "NetAdapter" "Networks" $env:COMPUTERNAME

#Replaces arp -a:

$NetNeighbor = Get-NetNeighbor | select InterfaceAlias, IPAddress, LinkLayerAddress 
PostData $PostURI $NetNeighbor $IsPostData "NetNeighbor" "Networks" $env:COMPUTERNAME

#Replaces netstat commands
$NetTCPConnect = Get-NetTCPConnection | Select-Object LocalAddress, LocalPort, RemoteAddress, RemotePort, State, OwningProcess,AppliedSetting, @{Name="Process";Expression={(Get-Process -Id $_.OwningProcess).ProcessName}}
PostData $PostURI $NetTCPConnect $IsPostData "NetTCPConnect" "Networks" $env:COMPUTERNAME


#Get Wi-fi Names and Passwords

$WlanPasswords = netsh.exe wlan show profiles | Select-String "\:(.+)$" | %{$wlanname=$_.Matches.Groups[1].Value.Trim(); $_} | %{(netsh wlan show profile name="$wlanname" key=clear)}  | Select-String 'Key Content\W+\:(.+)$' | %{$wlanpass=$_.Matches.Groups[1].Value.Trim(); $_} | %{[PSCustomObject]@{ PROFILE_NAME=$wlanname;PASSWORD=$wlanpass }}
PostData $PostURI $WlanPasswords $IsPostData "WlanPasswords" "Networks" $env:COMPUTERNAME


#Get Firewall Information. Replaces netsh firewall show config

$FirewallRule = Get-NetFirewallRule | select-object Name, DisplayName, Description, Direction, Action, EdgeTraversalPolicy, Owner, EnforcementStatus
PostData $PostURI $FirewallRule $IsPostData "FirewallRule" "Networks" $env:COMPUTERNAME

#Display active samba sessions

$SMBSessions = Get-SMBSession -ea silentlycontinue
PostData $PostURI $SMBSessions $IsPostData "SMBSessions" "Networks" $env:COMPUTERNAME

#Display active samba shares

$SMBShares = Get-SMBShare | select description, path, volume
PostData $PostURI $SMBShares $IsPostData "SMBShares" "Networks" $env:COMPUTERNAME
#Get IP routes to non-local destinations

$NetHops = Get-NetRoute | Where-Object -FilterScript { $_.NextHop -Ne "::" } | Where-Object -FilterScript { $_.NextHop -Ne "0.0.0.0" } | Where-Object -FilterScript { ($_.NextHop.SubString(0,6) -Ne "fe80::") } | select ifIndex,DestinationPrefix,NextHop,RouteMetric,ifMetric, @{Name='PreferredLifetime';Expression={$_.ConvertToDateTime($_.PreferredLifetime).ToString("yyyy/MM/dd HH:mm:ss")}}, @{Name='ValidLifetime';Expression={$_.ConvertToDateTime($_.ValidLifetime).ToString("yyyy/MM/dd HH:mm:ss")}}
PostData $PostURI $NetHops $IsPostData "NetHops" "Networks" $env:COMPUTERNAME

#Get network adapters that have IP routes to non-local destinations
$AdaptHops = Get-NetRoute | Where-Object -FilterScript {$_.NextHop -Ne "::"} | Where-Object -FilterScript { $_.NextHop -Ne "0.0.0.0" } | Where-Object -FilterScript { ($_.NextHop.SubString(0,6) -Ne "fe80::") } | Get-NetAdapter | select Publish,Protocol,Store,AddressFamily,State,ifIndex,Caption,Description,ElementName,InstanceID,AdminDistance,DestinationAddress,IsStatic,RouteMetric,TypeOfRoute,CompartmentId,DestinationPrefix,InterfaceAlias,InterfaceIndex,InterfaceMetric,NextHop, @{Name='PreferredLifetime';Expression={$_.ConvertToDateTime($_.PreferredLifetime).ToString("yyyy/MM/dd HH:mm:ss")}}, @{Name='ValidLifetime';Expression={$_.ConvertToDateTime($_.ValidLifetime).ToString("yyyy/MM/dd HH:mm:ss")}},PSComputerName
PostData $PostURI $AdaptHops $IsPostData "AdaptHops" "Networks" $env:COMPUTERNAME

#Get IP routes that have an infinite valid lifetime
$IpHops = Get-NetRoute | Where-Object -FilterScript { $_.ValidLifetime -Eq ([TimeSpan]::MaxValue) } | select Publish,Protocol,Store,AddressFamily,State,ifIndex,Caption,Description,ElementName,InstanceID,AdminDistance,DestinationAddress,IsStatic,RouteMetric,TypeOfRoute,CompartmentId,DestinationPrefix,InterfaceAlias,InterfaceIndex,InterfaceMetric,NextHop,@{Name='PreferredLifetime';Expression={$_.ConvertToDateTime($_.PreferredLifetime).ToString("yyyy/MM/dd HH:mm:ss")}}, @{Name='ValidLifetime';Expression={$_.ConvertToDateTime($_.ValidLifetime).ToString("yyyy/MM/dd HH:mm:ss")}},PSComputerName
PostData $PostURI $IpHops $IsPostData "IpHops" "Networks" $env:COMPUTERNAME


Write-Host -Fore Cyan "[!] Done"

#endregion

##################################################
#region User & Account Information               #
##################################################

Write-Host -Fore DarkCyan "[*] Gathering User & Account Information"

$getCurrentUser = Get-WMIObject -class Win32_ComputerSystem | select username 
$getCurrentPass = net user $env:USERNAME | Select-String "Password last set" 

$getCurrentUser | Add-Member -MemberType NoteProperty -Name "LastChangePassword" -Value  $getCurrentPass.ToString().Replace("Password last set","").Trim()
$currentuser = $getCurrentUser
PostData $PostURI $currentuser $IsPostData "currentuser" "Users_Accounts" $env:COMPUTERNAME

$systemname = Get-WmiObject -Class Win32_ComputerSystem | select Name, DNSHostName, Domain, Manufacturer, Model, PrimaryOwnerName, TotalPhysicalMemory, Workgroup
PostData $PostURI $systemname $IsPostData "systemname" "Users_Accounts" $env:COMPUTERNAME 

#$useraccounts = Get-WmiObject -Class Win32_UserAccount  | Select-Object -Property AccountType,Domain,LocalAccount,Name,PasswordRequired,SID,SIDType | ConvertTo-Html -fragment
$logonsession = Get-WmiObject -Class Win32_LogonSession | Select  LogonID,LogonType, @{NAME='StartTime';Expression={$_.ConvertToDateTime($_.StartTime).ToString("yyyy/MM/dd HH:mm:ss")}}
PostData $PostURI $logonsession $IsPostData "logonsession" "Users_Accounts" $env:COMPUTERNAME

#######ADDITIONS
#$currentlogonsession = query user | select USERNAME,SESSIONNAME,ID,STATE,"IDLE TIME","LOGON TIME"

$currentlogonsession = @()
$usersLines = @(query user) -split "\n"
foreach ($userLine in $usersLines)
{
    if ($userLine -match "USERNAME\s+SESSIONNAME\s+ID\s+STATE\s+IDLE TIME\s+LOGON TIME") {
        continue
    }
    $Parsed_User = $userLine -split '\s+'
    $currentlogonsession += New-Object PSCustomObject -Property @{
        USERNAME    = $Parsed_User[1]
        SESSIONNAME = $Parsed_User[2]
        ID          = $Parsed_User[3]
        STATE       = $Parsed_User[4]
        IDLE_TIME   = $Parsed_User[5]
        LOGON_TIME  = $Parsed_User[6]
    }
}


PostData $PostURI $currentlogonsession $IsPostData "currentlogonsession" "Users_Accounts" $env:COMPUTERNAME

$userprocesses = Get-Process -includeusername 
PostData $PostURI $userprocesses $IsPostData "userprocesses" "Users_Accounts" $env:COMPUTERNAME

$userprofiles = Get-WmiObject -Class Win32_UserProfile | Select Caption, LocalPath, SID, @{NAME='LastUseTime';Expression={$_.ConvertToDateTime($_.LastUseTime).ToString("yyyy/MM/dd HH:mm:ss")}}
PostData $PostURI $userprofiles $IsPostData "userprofiles" "Users_Accounts" $env:COMPUTERNAME

$administrators = Get-LocalGroupMember -Group "Administrators" 
PostData $PostURI $administrators $IsPostData "AdminGroups" "Users_Accounts" $env:COMPUTERNAME

$LocalGroup = Get-LocalGroup 
PostData $PostURI $LocalGroup $IsPostData "LocalGroup" "Users_Accounts" $env:COMPUTERNAME

#$LastChangePassword = New-Object -TypeName PSCustomObject
#$LastChangePassword | Add-Member -MemberType NoteProperty -Name "LastChangePassword" -Value  $lastChangePass.ToString().Replace("Password last set","").Trim()



Write-Host -Fore Cyan "[!] Done"

#endregion

##################################################
#region Installed Programs                       #
##################################################

Write-Host -Fore DarkCyan "[*] Gathering Installed Programs"

$InstProgs = Get-CimInstance -ClassName win32_product | Select Name, Version, Vendor,@{Name='InstallDate';Expression={$_.InstallDate.ToString("yyyy/MM/dd")}}, InstallSource, PackageName, LocalPackage 
PostData $PostURI $InstProgs $IsPostData "InstallPrograms" "InstalledPrograms" $env:COMPUTERNAME

$InstalledApps = Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Select DisplayName, DisplayVersion, Publisher, @{Name='InstallDate';Expression={$_.InstallDate.ToString("yyyy/MM/dd")}} 
PostData $PostURI $InstalledApps $IsPostData "UninstallPrograms" "InstalledPrograms" $env:COMPUTERNAME

Write-Host -Fore Cyan "[!] Done"

#endregion



##################################################
#region Checking other worthwhiles			     #
##################################################

Write-Host -Fore DarkCyan "[*] Running Peripheral Checks..."

#Logical drives (current session)
$LogicalDrives = get-wmiobject win32_logicaldisk | select DeviceID, DriveType, FreeSpace, Size, VolumeName
PostData $PostURI $LogicalDrives $IsPostData "LogicalDrives" "Peripheral" $env:COMPUTERNAME

#Gets list of USB devices

$USBDevices = Get-ItemProperty -Path HKLM:\System\CurrentControlSet\Enum\USB*\*\* | select FriendlyName, Driver, mfg, DeviceDesc
PostData $PostURI $USBDevices $IsPostData "USBDevices" "Peripheral" $env:COMPUTERNAME

#Identifies any connected/previously connected webcams
#$Imagedevice = Get-PnpDevice  -class 'image' -EA SilentlyContinue |  ConvertTo-Html -Fragment
$Imagedevice = Get-WmiObject Win32_PnPEntity | where {$_.caption -match 'camera'} -EA SilentlyContinue | where caption -match 'camera'
PostData $PostURI $Imagedevice $IsPostData "Imagedevice" "Peripheral" $env:COMPUTERNAME

#All currently connected PNP devices
$UPNPDevices = Get-PnpDevice -PresentOnly -class 'USB', 'DiskDrive', 'Mouse', 'Keyboard', 'Net', 'Image', 'Media', 'Monitor'
PostData $PostURI $UPNPDevices $IsPostData "UPNPDevices" "Peripheral" $env:COMPUTERNAME

#All previously connected disk drives not currently accounted for. Useful if target computer has had drive replaced/hidden
$UnknownDrives = Get-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Enum\USBSTOR\*\* | Select FriendlyName
PostData $PostURI $UnknownDrives $IsPostData "UnknownDrives" "Peripheral" $env:COMPUTERNAME

#Gets all link files created in last 180 days. Perhaps export this as a separate CSV and make it keyword searchable?

$LinkFiles = Get-WmiObject Win32_ShortcutFile | Where-Object {$_.LastModified -gt ((Get-Date).AddDays(-180)) } | sort LastModified -Descending | select Filename, Caption, @{NAME='CreationDate';Expression={$_.ConvertToDateTime($_.CreationDate).ToString("yyyy/MM/dd")}}, @{Name='LastAccessed';Expression={$_.ConvertToDateTime($_.LastAccessed).ToString("yyyy/MM/dd HH:mm:ss")}}, @{Name='LastModified';Expression={$_.ConvertToDateTime($_.LastModified).ToString("yyyy/MM/dd HH:mm:ss")}}, Target  
PostData $PostURI $LinkFiles $IsPostData "LinkFiles180days" "Peripheral" $env:COMPUTERNAME

#Gets last 100 days worth of Powershell History

$PSHistory = Get-History -count 500 | select id, commandline, startexecutiontime, endexecutiontime
PostData $PostURI $PSHistory $IsPostData "PSHistory500" "Peripheral" $env:COMPUTERNAME


#All items in Downloads folder. This may cause an error if the script is run from an external USB or Network drive, even when
$Downloads = Get-ChildItem C:\Users\*\Downloads\* -recurse  |  select  PSChildName, Root, Name, FullName, Extension, @{Name='CreationTimeUTC';Expression={$_.CreationTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastAccessTimeUTC';Expression={$_.LastAccessTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastWriteTimeUTC';Expression={$_.LastWriteTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , Attributes | where {$_.extension -eq '.exe'}
PostData $PostURI $Downloads $IsPostData "DownloadsFiles" "Peripheral" $env:COMPUTERNAME

#Executables Running From Obscure Places
$HiddenExecs1 = Get-ChildItem C:\Users\*\AppData\Local\Temp\* -recurse  |  select  PSChildName, Root, Name, FullName, Extension, @{Name='CreationTimeUTC';Expression={$_.CreationTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastAccessTimeUTC';Expression={$_.LastAccessTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastWriteTimeUTC';Expression={$_.LastWriteTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}}, Attributes | where {$_.extension -eq '.exe'}
PostData $PostURI $HiddenExecs1 $IsPostData "HiddenExecs1" "Peripheral" $env:COMPUTERNAME

$HiddenExecs2 = Get-ChildItem C:\Temp\* -recurse  |  select  PSChildName, Root, Name, FullName, Extension, @{Name='CreationTimeUTC';Expression={$_.CreationTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastAccessTimeUTC';Expression={$_.LastAccessTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastWriteTimeUTC';Expression={$_.LastWriteTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}}, Attributes | where {$_.extension -eq '.exe'}
PostData $PostURI $HiddenExecs2 $IsPostData "HiddenExecs2" "Peripheral" $env:COMPUTERNAME

$HiddenExecs3 = Get-ChildItem C:\PerfLogs\* -recurse  |  select  PSChildName, Root, Name, FullName, Extension, @{Name='CreationTimeUTC';Expression={$_.CreationTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastAccessTimeUTC';Expression={$_.LastAccessTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastWriteTimeUTC';Expression={$_.LastWriteTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}}, Attributes | where {$_.extension -eq '.exe'}
PostData $PostURI $HiddenExecs3 $IsPostData "HiddenExecs3" "Peripheral" $env:COMPUTERNAME

$HiddenExecs4 = Get-ChildItem C:\Users\*\Documents\* -recurse  |  select  PSChildName, Root, Name, FullName, Extension, @{Name='CreationTimeUTC';Expression={$_.CreationTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastAccessTimeUTC';Expression={$_.LastAccessTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}} , @{Name='LastWriteTimeUTC';Expression={$_.LastWriteTimeUTC.ToString("yyyy/MM/dd HH:mm:ss")}}, Attributes | where {$_.extension -eq '.exe'}
PostData $PostURI $HiddenExecs4 $IsPostData "HiddenExecs4" "Peripheral" $env:COMPUTERNAME

Write-Host -Fore Cyan "[!] Done"

#endregion

#####################################################################################################################################################################



###########################################################################################################
#region ########################## CREATING AND FORMATTING THE HTML FILES  ################################
###########################################################################################################

Write-Host -Fore DarkCyan "[*] Creating and Formatting our Index file"


# Setting Head for the index file
ConvertTo-Html -Head $head -Title "Live Forensic Output For $env:computername" >$FinalDes

# Setting up index style
$head = '<style> 
BODY{font-family:calibri; background-color: #f6ebf4;}
TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} 
TH{font-size:1.1em;color:#f6ebf4; border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color: #482673} 
TD{border-width:1px;padding: 2px;border-style: solid;border-color: black;background-color:white} 
</style>'

$head >> $FinalDes


# Making the Menus for Index File

$IndexNav = "'
<html>
<style>
ul#list-nav {
width:1200px;
margin:0 auto;
list-style:none;


}

ul#list-nav li {

float:left;
}

ul#list-nav li a {
  text-decoration:none;
  padding:5px 0;
  width:100%;
  background:#482673;
  color:#f6ebf4;
  float:left;
  text-align:center;
  border-left:2px solid #a2b3a1;
  border-top:2px solid #a2b3a1;
  border-bottom:2px solid #a2b3a1;
  display:block;
  font-size:20px
}

ul#list-nav li a:hover {
  background:#B73225;
  color:#ffff
}
</style>
<body>
<ul id='list-nav'>
  <li><a href='index.html'>Home</a></li>
  <li><a href='evtx.html'>Event Log Analysis</a></li>
  <li><a href='processes.html'>System Processes</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='users.html'>Users & Accounts</a></li>
  <li><a href='installed.html'>Installed programs</a></li>
  <li><a href='others.html'>Other Information</a></li>
</ul>
</body>
</html>'"

$BlackWidow ='
<div class="logo-container" align="center">
  <ul>


    <li>
      <div class="logo-holder logo-3">
        <a href="">
          <h3>Live Forensicator</h3>
          <p>A Black Widow Tool</p>
        </a>
      </div>
    </li>
    
  </ul>
</div>'

$BlackWidowStyle = '
<style>
@import url("https://fonts.googleapis.com/css?family=Bangers|Cinzel:400,700,900|Lato:100,300,400,700,900|Lobster|Lora:400,700|Mansalva|Muli:200,300,400,600,700,800,900|Open+Sans:300,400,600,700,800|Oswald:200,300,400,500,600,700|Roboto:100,300,400,500,700,900&display=swap");
/* Used Google Fonts */
font-family: "Roboto", sans-serif;
font-family: "Mansalva", cursive;
font-family: "Lato", sans-serif;
font-family: "Open Sans", sans-serif;
font-family: "Oswald", sans-serif;
font-family: "Lora", serif;
font-family: "Muli", sans-serif;
font-family: "Lobster", cursive;
font-family: "Cinzel", serif;
font-family: "Bangers", cursive;
/* Used Google Fonts */
*{
  margin:0;
  padding:0;
}
body{
  font-size:17px;
  color:#424242;
  font-family: "Open Sans", sans-serif;
  background-color: #ffffff;
  background-image: url("data:image/png;base64,");
}
h1, h2, h3, h4, h5, h6, p{
  margin:0px;
  padding:0px;
}
.logo-container ul {
    margin: 0;
    padding: 0;
    list-style: none;
    display:inline-block;
}
.logo-container ul li {
    width: 300px;
    height: 120px;
    background: #fff;
    border-radius: 10px;
    margin: 10px;
    float: left;
    padding:20px;
    box-shadow: 0px 5px 10px rgba(0, 0, 0, 0.25);    
    display: flex;
    align-items: center;
    justify-content: center;
}
.logo-container ul li a{
  text-decoration:none !important;
  display: inline-block;
}
.logo-holder{
  text-align:center;
}

/* Logo-3 */
.logo-3 h3 {
    color: #e74c3c;
    font-family: "Oswald", sans-serif;
    font-weight: 300;
    font-size: 50px;
    line-height:1.3;
}
.logo-3 p {
    font-size: 14px;
    letter-spacing: 7px;
    text-transform: uppercase;
    background: #34495e;
    font-weight: 400;
    color: #fff;
    padding-left: 5px;
}


</style>'

$BlackWidowStyle >> $FinalDes
$BlackWidow >> $FinalDes
$IndexNav >> $FinalDes

'<br><br>' >> $FinalDes

# Setting Body content for index file.
echo "<h2><u><center>Live Forensics Result for $env:computername</center></u></h2>"                                | Out-File -Append $FinalDes

'<br><br>' >> $FinalDes

#Case information

echo "<h2><center> Case reference: $CASENO </center></h2><br>"                                                        | Out-File -Append $FinalDes

echo "<h2><center> Examiner Name: $Handler </center></h2><br>"                                                     | Out-File -Append $FinalDes

echo "<h2><center>Exhibit reference: $Ref </center></h2>"                                                       | Out-File -Append $FinalDes

echo "<h2><center>Device: $Des </center></h2><br>"                                                          | Out-File -Append $FinalDes

echo "<h2><center>Examination Location: $Loc </center></h2><br>"                                               | Out-File -Append $FinalDes

echo "<h3><center> Start Time and Date: $StartTime </center></h3><br>"                                            | Out-File -Append $FinalDes
echo "<h3><center> End Time and Date: $EndTime </h3><br>"                                                      | Out-File -Append $FinalDes

'<br><br>' >> $FinalDes
'<br><br>' >> $FinalDes

#endregion


#############################################################################################################
#region   EVENT LOG ANALYSIS HTML FILES  ####################################################################
#############################################################################################################

# Making the head for evtx.html
ConvertTo-Html -Head $head -Title "Live Forensic Output For $env:computername" > $EVTXDes

# Header style for System Page

$head = '<style> 
BODY{font-family:calibri; background-color: #f6ebf4;}
TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} 
TH{font-size:1.1em;color:#f6ebf4; border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color: #482673} 
TD{border-width:1px;padding: 2px;border-style: solid;border-color: black;background-color:white} 
</style>'

$head >> $EVTXDes


# Making the menus for system.html

$EVTXNav = "'
<html>
<style>
ul#list-nav {
width:1200px;
margin:0 auto;
list-style:none;
}

ul#list-nav li {

float:left;
}

ul#list-nav li a {
  text-decoration:none;
  padding:5px 0;
  width:100%;
  background:#482673;
  color:#f6ebf4;
  float:left;
  text-align:center;
  border-left:2px solid #a2b3a1;
  border-top:2px solid #a2b3a1;
  border-bottom:2px solid #a2b3a1;
  border-right:2px solid #a2b3a1;
  display:block;
  font-size:20px
}

ul#list-nav li a:hover {
  background:#B73225;
  color:#ffff
}


</style>
<body>
<ul id='list-nav'>
  <li><a href='index.html'>Home</a></li>
  <li><a href='evtx.html'>Event Log Analysis</a></li>
  <li><a href='processes.html'>System Processes</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='others.html'>Other Information</a></li>
</ul>
</body>
</html>
'"

$EVTXNav >> $EVTXDes

'<br><br>' >> $EVTXDes



echo "<h2><u>WINDOWS EVENT LOG ANALYSIS</u></h2> "                                                                                 | Out-File -Append $EVTXDes
#echo "<h3><u><center>CHECK SERVICES </center></u></h3> "                                                         | Out-File -Append $EVTXDes

'<details>' >> $EVTXDes
echo "<summary>Check service 01</summary>"                                                   | Out-File -Append $EVTXDes
if ($ServiceLogsEvent) {
    $ServiceLogsEventData = $ServiceLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$ServiceLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes


#$DeleteLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check delete log 02 </summary>"                                                   | Out-File -Append $EVTXDes
if ($DeleteLogsEvent) {
    $DeleteLogsEventData = $DeleteLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$DeleteLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#StopServiceLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check stop service action 03</summary>"                                                   | Out-File -Append $EVTXDes
if ($StopServiceLogsEvent) {
    $StopServiceLogsEventData = $StopServiceLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$StopServiceLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#UnkownCommandParameterLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check UnkownCommand Event 04</summary>"                                                   | Out-File -Append $EVTXDes
if ($UnkownCommandParameterLogsEvent) {
    $UnkownCommandParameterLogsEventData = $UnkownCommandParameterLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$UnkownCommandParameterLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#PowershellCommandLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check PowershellCommand Event 05</summary>"                                                   | Out-File -Append $EVTXDes
if ($PowershellCommandLogsEvent) {
    $PowershellCommandLogsEventData = $PowershellCommandLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$PowershellCommandLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#RegistryLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check Register Event 06</summary>"                                                   | Out-File -Append $EVTXDes
if ($RegistryLogsEvent) {
    $RegistryLogsEventData = $RegistryLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$RegistryLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#FileLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check File log Event 07</summary>"                                                   | Out-File -Append $EVTXDes
if ($FileLogsEvent) {
    $FileLogsEventData = $FileLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$FileLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#ShareResourceLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check Share log resource Event 08</summary>"                                                   | Out-File -Append $EVTXDes
if ($ShareResourceLogsEvent) {
    $ShareResourceLogsEventData = $ShareResourceLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$ShareResourceLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#WMIConnectLogs
'<details>' >> $EVTXDes
echo "<summary>Check Connect from PC to other PC Event 09</summary>"                                                   | Out-File -Append $EVTXDes
if ($WMIConnectLogsEvent) {
    $WMIConnectLogsEventData = $WMIConnectLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$WMIConnectLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#TaskcheduleLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check Task schedule Event 10</summary>"                                                   | Out-File -Append $EVTXDes
if ($TaskcheduleLogsEvent) {
    $TaskcheduleLogsEventData = $TaskcheduleLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$TaskcheduleLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#ScTcbPrivilegeEvent
'<details>' >> $EVTXDes
echo "<summary>Check action ScTcbPrivilege Event 11</summary>"                                                   | Out-File -Append $EVTXDes
if ($ScTcbPrivilegeEvent) {
    $ScTcbPrivilegeEventData = $ScTcbPrivilegeEvent | ConvertTo-Html -Fragment
    echo "<table>$ScTcbPrivilegeEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#LogonLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check Logon Event 12</summary>"                                                   | Out-File -Append $EVTXDes
if ($LogonLogsEvent) {
    $LogonLogsEventData = $LogonLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$LogonLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#LogonFailLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check Login fail Event 13</summary>"                                                   | Out-File -Append $EVTXDes
if ($LogonFailLogsEvent) {
    $LogonFailLogsEventData = $LogonFailLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$LogonFailLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#ChangeUserLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check change user info Event 14</summary>"                                                   | Out-File -Append $EVTXDes
if ($ChangeUserLogsEvent) {
    $ChangeUserLogsEventData = $ChangeUserLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$ChangeUserLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#ChangeGroupLogsEvent
'<details>' >> $EVTXDes
echo "<summary> Check change group Event 15</summary>" | Out-File -Append $EVTXDes
if ($ChangeGroupLogsEvent) {
    $ChangeGroupLogsEventData = $ChangeGroupLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$ChangeGroupLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#ActiveDirectoryLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check Active directory Event 16</summary>"  | Out-File -Append $EVTXDes
if ($ActiveDirectoryLogsEvent) {
    $ActiveDirectoryLogsEventData = $ActiveDirectoryLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$ActiveDirectoryLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

#ChangeTimeLogsEvent
'<details>' >> $EVTXDes
echo "<summary>Check Change system time Event 17</summary>" | Out-File -Append $EVTXDes
if ($ChangeTimeLogsEvent) {
    $ChangeTimeLogsEventData = $ChangeTimeLogsEvent | ConvertTo-Html -Fragment
    echo "<table>$ChangeTimeLogsEventData  </table><br> "                                                         | Out-File -Append $EVTXDes
}
'</details>' >> $EVTXDes
'<br><br>' >> $EVTXDes

Write-Host -Fore Cyan "[!] Done"

#endregion

#############################################################################################################
#region   PROCESSES | SCHEDULED TASK | REGISTRY    ##########################################################
#############################################################################################################
# Making the head for processes.html
ConvertTo-Html -Head $head -Title "Live Forensic Output For $env:computername" >$ProcDes

# Header style for System Page

$head = '<style> 
BODY{font-family:calibri; background-color: #f6ebf4;}
TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} 
TH{font-size:1.1em;color:#f6ebf4; border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color: #482673} 
TD{border-width:1px;padding: 2px;border-style: solid;border-color: black;background-color:white} 
</style>'

$head >> $ProcDes


# Making the menus for system.html

$ProcNav = "'
<html>
<style>
ul#list-nav {
width:1200px;
margin:0 auto;
list-style:none;
}

ul#list-nav li {

float:left;
}

ul#list-nav li a {
  text-decoration:none;
  padding:5px 0;
  width:100%;
  background:#482673;
  color:#f6ebf4;
  float:left;
  text-align:center;
  border-left:2px solid #a2b3a1;
  border-top:2px solid #a2b3a1;
  border-bottom:2px solid #a2b3a1;
  display:block;
  font-size:20px
}

ul#list-nav li a:hover {
  background:#B73225;
  color:#ffff
}
</style>
<body>
<ul id='list-nav'>
  <li><a href='index.html'>Home</a></li>
  <li><a href='evtx.html'>Event Log Analysis</a></li>
  <li><a href='processes.html'>System Processes</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='others.html'>Other Information</a></li>
</ul>
</body>
</html>
'"

$ProcNav >> $ProcDes

'<br><br>' >> $ProcDes


echo "<h2><u>SERVICES, PROCESSES & TASKSCHEDULER </u></h2>"                                                      | Out-File -Append $ProcDes

#Services
'<details>' >> $ProcDes
echo "<summary>Service List 19</summary>"                                                                | Out-File -Append $ProcDes
if ($Services) {
    $ServicesData = $Services | ConvertTo-Html -Fragment
    echo "<table>$ServicesData</table><br> "                                                              | Out-File -Append $ProcDes
}
'</details>' >> $ProcDes
'<br><br>' >> $ProcDes



'<details>' >> $ProcDes
echo "<summary>Process List 20</summary>"                                                                | Out-File -Append $ProcDes
if ($Processes) {
    $ProcessesData = $Processes | ConvertTo-Html -Fragment
    echo "<table>$ProcessesData</table><br> "                                                              | Out-File -Append $ProcDes
}
'</details>' >> $ProcDes
'<br><br>' >> $ProcDes

#StartupProgs
'<details>' >> $ProcDes
echo "<summary>Items set to run on startup 21</summary>"                                                                | Out-File -Append $ProcDes
if ($StartupProgs) {
    $StartupProgsData = $StartupProgs | ConvertTo-Html -Fragment
    echo "<table>$StartupProgsData</table><br> "                                                              | Out-File -Append $ProcDes
}
'</details>' >> $ProcDes
'<br><br>' >> $ProcDes


#ScheduledTask
'<details>' >> $ProcDes
echo "<summary>Scheduled Tasks 22</summary>"                                                                | Out-File -Append $ProcDes
if ($ScheduledTask) {
    $ScheduledTaskData = $ScheduledTask | ConvertTo-Html -Fragment
    echo "<table>$ScheduledTaskData</table><br> "                                                              | Out-File -Append $ProcDes
}
'</details>' >> $ProcDes
'<br><br>' >> $ProcDes


'<details>' >> $ProcDes
echo "<summary>Scheduled Tasks & State</summary>"   | Out-File -Append $ProcDes
if ($ScheduledTask2) {
    $ScheduledTask2Data = $ScheduledTask2 | ConvertTo-Html -Fragment
    echo "<table>$ScheduledTask2Data</table><br> "                                                              | Out-File -Append $ProcDes
}
'</details>' >> $ProcDes
'<br><br>' >> $ProcDes


#endregion

#############################################################################################################
#region   NETWORKS SECTION     ##############################################################################
#############################################################################################################

# Making the head for network.html
ConvertTo-Html -Head $head -Title "Live Forensic Output For $env:computername"  >$NetDes

# Header style for Network Page

$head = '<style> 
BODY{font-family:calibri; background-color: #f6ebf4;}
TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} 
TH{font-size:1.1em;color:#f6ebf4; border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color: #482673} 
TD{border-width:1px;padding: 2px;border-style: solid;border-color: black;background-color:white} 
</style>'

$head >> $NetDes


# Making the menus for network.html

$NetNav = "'
<html>
<style>
ul#list-nav {
width:1200px;
margin:0 auto;
list-style:none;
}

ul#list-nav li {

float:left;
}

ul#list-nav li a {
  text-decoration:none;
  padding:5px 0;
  width:100%;
  background:#482673;
  color:#f6ebf4;
  float:left;
  text-align:center;
  border-left:2px solid #a2b3a1;
  border-top:2px solid #a2b3a1;
  border-bottom:2px solid #a2b3a1;
  display:block;
  font-size:20px
}

ul#list-nav li a:hover {
  background:#B73225;
  color:#ffff
}
</style>
<body>
<ul id='list-nav'>
  <li><a href='index.html'>Home</a></li>
  <li><a href='evtx.html'>Event Log Analysis</a></li>
  <li><a href='processes.html'>System Processes</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='users.html'>Users & Accounts</a></li>
  <li><a href='installed.html'>Installed programs</a></li>
  <li><a href='others.html'>Other Information</a></li>
</ul>
</body>
</html>
'"

$NetNav >> $NetDes

'<br><br>' >> $NetDes





echo "<h2><u>Network Information</u></h2>"'' >> $NetDes


'<details>' >> $NetDes
echo "<summary>Network Adapter Information</summary>"                                                               | Out-File -Append $NetDes
if ($NetworkAdapter) { 
    $NetworkAdapterData = $NetworkAdapter | ConvertTo-Html -Fragment 
    echo "<table>$NetworkAdapterData</table><br> "                                                              | Out-File -Append $NetDes
  }
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Current IP Configuration</summary>"                                                                | Out-File -Append $NetDes
if ($IPConfiguration) {
    $IPConfigurationData = $IPConfiguration | ConvertTo-Html -Fragment 
    echo "<table>$IPConfigurationData</table><br> "                                                              | Out-File -Append $NetDes
  }
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Network Adapter IP Addresses - IPv4 and v6</summary>"                                                                | Out-File -Append $NetDes
if ($NetIPaddress) {
    $NetIPaddressData = $NetIPaddress | ConvertTo-Html -Fragment 
    echo "<table>$NetIPaddressData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Current Connection Profiles</summary>"                                                                | Out-File -Append $NetDes
if ($NetConnectProfile)
{
    $NetConnectProfileData = $NetConnectProfile | ConvertTo-Html -Fragment 
    echo "<table>$NetConnectProfileData</table><br> "                                                              | Out-File -Append $NetDes
}

'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Associated WiFi Networks and Passwords</summary>"                                                                | Out-File -Append $NetDes
if ($WlanPasswords)
{
    $WlanPasswordsData = $WlanPasswords | ConvertTo-Html -Fragment 
    echo "<table>$WlanPasswordsData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Address Resolution Protocol Cache</summary>"                                                                | Out-File -Append $NetDes
if ($NetNeighbor)
{
    $NetNeighborData = $NetNeighbor | ConvertTo-Html -Fragment 
    echo "<table>$NetNeighborData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Current TCP Connections and Associated Processes 23</summary>"                                                                | Out-File -Append $NetDes
if ($NetTCPConnect)
{
    $NetTCPConnectData = $NetTCPConnect | ConvertTo-Html -Fragment 
    echo "<table>$NetTCPConnectData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>DNS Cache</summary>"                                                                | Out-File -Append $NetDes
if ($DNSCache)
{
    $DNSCacheData = $DNSCache | ConvertTo-Html -Fragment 
    echo "<table>$DNSCacheData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Current Firewall Rules</summary>"                                                                | Out-File -Append $NetDes
if ($FirewallProfile)
{
    $FirewallProfileData = $FirewallProfile | ConvertTo-Html -Fragment 
    echo "<table>$FirewallProfileData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Active SMB sessions (if this device is a server)</summary>"                                                                | Out-File -Append $NetDes
if ($SMBSessions)
{
    $SMBSessionsData = $SMBSessions | ConvertTo-Html -Fragment 
    echo "<table>$SMBSessionsData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Active SMB Shares on this device</summary>"                                                                | Out-File -Append $NetDes
if ($SMBShares)
{
    $SMBSharesData = $SMBShares | ConvertTo-Html -Fragment 
    echo "<table>$SMBSharesData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>IP Routes to non local Destinations</summary>"                                                                | Out-File -Append $NetDes
if ($NetHops)
{
    $NetHopsData = $NetHops | ConvertTo-Html -Fragment 
    echo "<table>$NetHopsData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>Network Adapters with IP Routes to non Local Destination</summary>"                                                                | Out-File -Append $NetDes
if ($AdaptHops)
{
    $AdaptHopsData = $AdaptHops | ConvertTo-Html -Fragment 
    echo "<table>$AdaptHopsData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes




'<details>' >> $NetDes
echo "<summary>IP Routes with infinite valid lifetime</summary>"                                                                | Out-File -Append $NetDes
if ($IpHops)
{
    $IpHopsData = $IpHops | ConvertTo-Html -Fragment 
    echo "<table>$IpHopsData</table><br> "                                                              | Out-File -Append $NetDes
}
'</details>' >> $NetDes
'<br><br>' >> $NetDes
'<br><br>'>> $NetDes

'<center>' >> $NetDes
echo "<h3> Evidence gathered from  $env:computername  by  $operator at: $EndTime with: NOCMT >Live Forensicator </a> </h3>" | Out-File -Append $NetDes
'</center>' >> $NetDes

#endregion

#############################################################################################################
#region   OTHER NOTABLE CHECKS         ######################################################################
#############################################################################################################

# Making the head for others.html
ConvertTo-Html -Head $head -Title "Live Forensic Output For $env:computername" > $OtherDes

# Header style for System Page

$head = '<style> 
BODY{font-family:calibri; background-color: #f6ebf4;}
TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} 
TH{font-size:1.1em;color:#f6ebf4; border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color: #482673} 
TD{border-width:1px;padding: 2px;border-style: solid;border-color: black;background-color:white} 
</style>'

$head >> $OtherDes


# Making the menus for system.html

$OtherNav = "'
<html>
<style>
ul#list-nav {
width:1200px;
margin:0 auto;
list-style:none;
}

ul#list-nav li {

float:left;
}

ul#list-nav li a {
  text-decoration:none;
  padding:5px 0;
  width:100%;
  background:#482673;
  color:#f6ebf4;
  float:left;
  text-align:center;
  border-left:2px solid #a2b3a1;
  border-top:2px solid #a2b3a1;
  border-bottom:2px solid #a2b3a1;
  border-right:2px solid #a2b3a1;
  display:block;
  font-size:20px
}

ul#list-nav li a:hover {
  background:#B73225;
  color:#ffff
}
</style>
<body>
<ul id='list-nav'>
  <li><a href='index.html'>Home</a></li>
  <li><a href='evtx.html'>Event Log Analysis</a></li>
  <li><a href='processes.html'>System Processes</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='users.html'>Users & Accounts</a></li>
  <li><a href='installed.html'>Installed programs</a></li>
  <li><a href='others.html'>Other Information</a></li>
</ul>
</body>
</html>
'"

$OtherNav >> $OtherDes

'<br><br>' >> $OtherDes




echo "<h2><u>OTHER NOTABLE CHECKS</u></h2> "                                                                              | Out-File -Append $OtherDes

'<details>' >> $OtherDes
echo "<summary>Logical Drives </summary>"                                                                | Out-File -Append $OtherDes
if ($LogicalDrives) {
    $LogicalDrivesData = $LogicalDrives | ConvertTo-Html -Fragment
    echo "<table>$LogicalDrivesData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>Connected & Disconnected Webcams </summary>"                                                                | Out-File -Append $OtherDes
if ($Imagedevice) {
    $ImagedeviceData = $Imagedevice | ConvertTo-Html -Fragment
    echo "<table>$ImagedeviceData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>USB Devices </summary>"                                                                | Out-File -Append $OtherDes
if ($USBDevices) {
    $USBDevicesData = $USBDevices | ConvertTo-Html -Fragment
    echo "<table>$USBDevicesData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>UPNPDevices </summary>"                                                                | Out-File -Append $OtherDes
if ($UPNPDevices) {
    $UPNPDevicesData = $UPNPDevices | ConvertTo-Html -Fragment
    echo "<table>$UPNPDevicesData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>All Previously Connected Drives </summary>"                                                                | Out-File -Append $OtherDes
if ($UnknownDrives) {
    $UnknownDrivesData = $UnknownDrives | ConvertTo-Html -Fragment
    echo "<table>$UnknownDrivesData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes


'<details>' >> $OtherDes
echo "<summary>All Files Created in the last 180days </summary>"                                                                | Out-File -Append $OtherDes
if ($UnknownDrives) {
    $UnknownDrivesData = $UnknownDrives | ConvertTo-Html -Fragment
    echo "<table>$UnknownDrivesData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>All Files Created in the last 180days </summary>"                                                                | Out-File -Append $OtherDes
if ($LinkFiles) {
    $LinkFilesData = $LinkFiles | ConvertTo-Html -Fragment
    echo "<table>$LinkFilesData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>500Days Powershell History </summary>"                                                                | Out-File -Append $OtherDes
if ($PSHistory) {
    $PSHistoryData = $PSHistory | ConvertTo-Html -Fragment
    echo "<table>$PSHistoryData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>Executables in the Downloads folder </summary>"                                                                | Out-File -Append $OtherDes
if ($Downloads) {
    $DownloadsData = $Downloads | ConvertTo-Html -Fragment
    echo "<table>$DownloadsData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>Executables In AppData </summary>"                                                                | Out-File -Append $OtherDes
if ($HiddenExecs1) {
    $HiddenExecs1Data = $HiddenExecs1 | ConvertTo-Html -Fragment
    echo "<table>$HiddenExecs1Data</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>Executables In Temp</summary>"                                                                | Out-File -Append $OtherDes
if ($HiddenExecs2) {
    $HiddenExecs2Data = $HiddenExecs2 | ConvertTo-Html -Fragment
    echo "<table>$HiddenExecs2Data</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>Executables In Perflogs</summary>"                                                                | Out-File -Append $OtherDes
if ($HiddenExecs3) {
    $HiddenExecs3Data = $HiddenExecs3 | ConvertTo-Html -Fragment
    echo "<table>$HiddenExecs3Data</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>Executables In Documents Folder</summary>"                                                                | Out-File -Append $OtherDes
if ($HiddenExecs4) {
    $HiddenExecs4Data = $HiddenExecs4 | ConvertTo-Html -Fragment
    echo "<table>$HiddenExecs4Data</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes



'<details>' >> $OtherDes
echo "<summary>Files with same extension as well-known ransomware encrypted files </summary>"                        | Out-File -Append $OtherDes
if ($FindFiles) {
    $FindFilesData = $FindFiles | ConvertTo-Html -Fragment
    echo "<table>$FindFilesData</table><br> "                                                              | Out-File -Append $OtherDes
}
'</details>' >> $OtherDes
'<br><br>' >> $OtherDes


'<br><br>' >> $OtherDes

'<center>' >> $OtherDes
echo "<h3> Evidence gathered from  $env:computername  by  $operator at: $Endtimecheck with: NOCMT >Live Forensicator</a> </h3>" | Out-File -Append $OtherDes
'</center>' >> $OtherDes


#endregion

#############################################################################################################
#region   USER & ACCOUNTS SECTION     #######################################################################
#############################################################################################################

# Making the head for users.html
ConvertTo-Html -Head $head -Title "Live Forensic Output For $env:computername"  >$UserDes

# Header style for Network Page

$head = '<style> 
BODY{font-family:calibri; background-color: #f6ebf4;}
TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} 
TH{font-size:1.1em;color:#f6ebf4; border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color: #482673} 
TD{border-width:1px;padding: 2px;border-style: solid;border-color: black;background-color:white} 
</style>'

$head >> $UserDes


# Making the menus for network.html

$UserNav = "'
<html>
<style>
ul#list-nav {
width:1200px;
margin:0 auto;
list-style:none;
}

ul#list-nav li {

float:left;
}

ul#list-nav li a {
  text-decoration:none;
  padding:5px 0;
  width:100%;
  background:#482673;
  color:#f6ebf4;
  float:left;
  text-align:center;
  border-left:2px solid #a2b3a1;
  border-top:2px solid #a2b3a1;
  border-bottom:2px solid #a2b3a1;
  display:block;
  font-size:20px
}

ul#list-nav li a:hover {
  background:#B73225;
  color:#ffff
}
</style>
<body>
<ul id='list-nav'>
  <li><a href='index.html'>Home</a></li>
  <li><a href='evtx.html'>Event Log Analysis</a></li>
  <li><a href='processes.html'>System Processes</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='users.html'>Users & Accounts</a></li>
  <li><a href='installed.html'>Installed programs</a></li>
  <li><a href='others.html'>Other Information</a></li>
</ul>
</body>
</html>
'"

$UserNav >> $UserDes

'<br><br>' >> $UserDes





echo "<h2><u>User(s) Information</u></h2>"                                                                                   | Out-File -Append $UserDes


'<details>' >> $UserDes
echo "<summary>Current User Information </summary>"                                                                | Out-File -Append $UserDes
if ($currentuser) { 
    $currentuserData = $currentuser | ConvertTo-Html -Fragment 
    echo "<table>  $currentuserData</table><br> "                                                              | Out-File -Append $UserDes
}
'</details>' >> $UserDes
'<br><br>' >> $UserDes






'<details>' >> $UserDes
echo "<summary>System Details </summary>"                                                                | Out-File -Append $UserDes
if ($systemname) {
    $systemnameData = $systemname | ConvertTo-Html -Fragment 
    echo "<table>$systemnameData</table><br> "                                                              | Out-File -Append $UserDes
 }
'</details>' >> $UserDes
'<br><br>' >> $UserDes



'<details>' >> $UserDes
echo "<summary>Logon Sessions</summary>"                                                                | Out-File -Append $UserDes
if ($logonsession) {
    $logonsessionData = $logonsession | ConvertTo-Html -Fragment 
    echo "<table>$logonsessionData</table><br> "                                                              | Out-File -Append $UserDes
}
'</details>' >> $UserDes
'<br><br>' >> $UserDes

'<details>' >> $UserDes
echo "<summary>Current Logon Sessions</summary>"                                                                | Out-File -Append $UserDes
if ($currentlogonsession) {
    $currentlogonsessionData = $currentlogonsession | ConvertTo-Html -Fragment 
    echo "<table>$currentlogonsessionData</table><br> "                                                              | Out-File -Append $UserDes
}
'</details>' >> $UserDes
'<br><br>' >> $UserDes


'<details>' >> $UserDes
echo "<summary>User Profile</summary>"                                                                | Out-File -Append $UserDes
if ($userprofiles) {
   $userprofilesData = $userprofiles | ConvertTo-Html -Fragment 
   echo "<table>$userprofilesData</table><br> "                                                              | Out-File -Append $UserDes
}
'</details>' >> $UserDes
'<br><br>' >> $UserDes




'<details>' >> $UserDes
echo "<summary>Administrator Accounts</summary>"                                                                | Out-File -Append $UserDes
if ($administrators) {
    $administratorsData = $administrators | ConvertTo-Html -Fragment 
    echo "<table>$administratorsData</table><br> "                                                              | Out-File -Append $UserDes
}
'</details>' >> $UserDes
'<br><br>' >> $UserDes



'<details>' >> $UserDes
echo "<summary>Local Groups</summary>"                                                                | Out-File -Append $UserDes
if ($LocalGroup) {
    $LocalGroupData = $LocalGroup | ConvertTo-Html -Fragment 
    echo "<table>$LocalGroupData</table><br> "                                                              | Out-File -Append $UserDes
}
'</details>' >> $UserDes
'<br><br>' >> $UserDes




'<br><br>'>> $UserDes

'<center>' >> $UserDes
echo "<h3> Evidence gathered from  $env:computername  by  $operator at: $EndTime with: NOCMT >Live Forensicator </a> </h3>"  | Out-File -Append $UserDes
'</center>' >> $UserDes

#endregion





#############################################################################################################
#region   INSTALLED PROGRAMS SECTION     ####################################################################
#############################################################################################################

# Making the head for users.html
ConvertTo-Html -Head $head -Title "Live Forensic Output For $env:computername"  > $InstallProDes

# Header style for Network Page

$head = '<style> 
BODY{font-family:calibri; background-color: #f6ebf4;}
TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} 
TH{font-size:1.1em;color:#f6ebf4; border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color: #482673} 
TD{border-width:1px;padding: 2px;border-style: solid;border-color: black;background-color:white} 
</style>'

$head >> $InstallProDes


# Making the menus for network.html

$UserNav = "'
<html>
<style>
ul#list-nav {
width:1200px;
margin:0 auto;
list-style:none;
}

ul#list-nav li {

float:left;
}

ul#list-nav li a {
  text-decoration:none;
  padding:5px 0;
  width:100%;
  background:#482673;
  color:#f6ebf4;
  float:left;
  text-align:center;
  border-left:2px solid #a2b3a1;
  border-top:2px solid #a2b3a1;
  border-bottom:2px solid #a2b3a1;
  display:block;
  font-size:20px
}

ul#list-nav li a:hover {
  background:#B73225;
  color:#ffff
}
</style>
<body>
<ul id='list-nav'>
  <li><a href='index.html'>Home</a></li>
  <li><a href='evtx.html'>Event Log Analysis</a></li>
  <li><a href='processes.html'>System Processes</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='users.html'>Users & Accounts</a></li>
  <li><a href='installed.html'>Installed programs</a></li>
  <li><a href='others.html'>Other Information</a></li>
</ul>
</body>
</html>
'"

$UserNav >> $InstallProDes

'<br><br>' >> $InstallProDes


echo "<h2><u>Installed Programs</u></h2>"                                                                                   | Out-File -Append $InstallProDes

'<details>' >> $InstallProDes
echo "<summary>Installed programs Information 26</summary>"                                                                | Out-File -Append $InstallProDes
if ($InstProgs) {
    $InstProgsData = $InstProgs | ConvertTo-Html -Fragment
    echo "<table>  $InstProgsData </table><br> "                                                              | Out-File -Append $InstallProDes
}
'</details>' >> $InstallProDes
'<br><br>' >> $InstallProDes

'<details>' >> $InstallProDes
echo "<summary>Uninstalled programs Information </summary>"                                                                | Out-File -Append $InstallProDes
if ($InstalledApps) {
    $InstalledAppsData = $InstalledApps | ConvertTo-Html -Fragment
    echo "<table>  $InstalledAppsData </table><br> "                                                              | Out-File -Append $InstallProDes
}
'</details>' >> $InstallProDes
'<br><br>' >> $InstallProDes

#endregion






###################################################################################################
#region   Upload FtpServer     ####################################################################
###################################################################################################

if ($IsUploadFtp -eq $true) {
    ValidateFtpFolder  $FtpUser $FtpPass $FtpPath
    UploadFtpServer $FtpUser $FtpPass $FtpPath
}
#endregion